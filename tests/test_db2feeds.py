...
class Db2FeedsTest(DbTestCase):
    ...
    def test_convert__existing(self):
    ...
    def test_convert__entry(self):
        ...
        with engine.connect() as connection:
            ...
            entry = list(table.get_entries())[0]

            self.assertEqual(entry.link, "https://www.youtube.com/feeds/videos.xml?channel_id=12345678")
            self.assertTrue(entry.date_created)
            #self.assertTrue(entry.body_hash)
            #self.assertTrue(entry.meta_hash)
